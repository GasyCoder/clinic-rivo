...
<template>
    <nav class="sticky top-16 z-[1020] flex max-w-full gap-1 overflow-x-auto rounded-lg border border-gray-200 bg-white/95 p-2 shadow-sm backdrop-blur dark:border-gray-900 dark:bg-gray-950/95" aria-label="Sections Pharmacie">
        ...
        <component
            :is="linkMode ? Link : 'button'"
            v-if="capabilities.can_view_care_consumables"
            :href="linkMode ? '/pharmacy?tab=care-consumables' : undefined"
            :type="linkMode ? undefined : 'button'"
            :class="['inline-flex shrink-0 items-center gap-2 rounded px-4 py-2.5 text-xs font-bold transition', active === 'care-consumables' ? 'bg-slate-700 text-white shadow-sm dark:bg-white dark:text-slate-800' : 'bg-gray-100 text-slate-500 hover:text-slate-700 dark:bg-gray-900 dark:text-slate-300 dark:hover:text-white']"
            @click="!linkMode && emit('select', 'care-consumables')"
        >
            <Icon name="user-check" />Consommables Soins
            <span v-if="careConsumableCount" :class="['rounded-full px-1.5 py-0.5 text-[10px]', active === 'care-consumables' ? 'bg-white/20 text-white dark:bg-slate-700' : 'bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300']">{{ careConsumableCount }}</span>
        </component>
        <component
            :is="linkMode ? Link : 'button'"
