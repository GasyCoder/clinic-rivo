...
const inputClass = 'h-11 w-full rounded-lg border border-gray-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-100 disabled:bg-gray-50 disabled:text-slate-500 dark:border-gray-800 dark:bg-gray-950 dark:text-white dark:disabled:bg-gray-900';
...
<template>
    <form class="space-y-5" @submit.prevent="submit">
        ...
        <div v-if="form.supplier_catalog_item_uuid" class="flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900 dark:border-emerald-900 dark:bg-emerald-950/20 dark:text-emerald-100">
            <Icon name="link" class="mt-0.5 text-lg" />
        ...
        <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-900 dark:bg-gray-950">
            <h2 class="font-heading text-base font-bold text-slate-800 dark:text-white">Identité du médicament</h2>
            <div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <label class="block">
                    ...
                    <span v-if="editing" class="mt-1 block text-xs text-slate-400">Le code ne change pas : les fichiers d’import le désignent.</span>
                </label>
                <label class="block lg:col-span-2"><span :class="labelClass">Nom standard à la clinique <span class="text-red-500">*</span></span><input v-model="form.name" name="name" :class="inputClass" required><span class="mt-1 block text-xs text-slate-400">Le nom sous lequel il est vendu et suivi, quel que soit le nom donné par chaque fournisseur.</span></label>
        ...
        <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-900 dark:bg-gray-950">
            <h2 class="font-heading text-base font-bold text-slate-800 dark:text-white">Classement et stock</h2>
            <div class="mt-4 grid gap-4 sm:grid-cols-3">
                ...
                <label class="flex items-center gap-3 self-end rounded-lg border border-gray-200 px-3 py-3 text-sm text-slate-700 dark:border-gray-800 dark:text-slate-200"><input v-model="form.prescription_required" type="checkbox" class="h-4 w-4">Délivré sur ordonnance</label>
            </div>
            <fieldset v-if="suppliers.length" class="mt-4">
                <legend :class="labelClass">Fournisseurs habituels</legend>
                <div class="flex flex-wrap gap-2">
                    <label v-for="supplier in suppliers" :key="supplier.uuid" class="flex items-center gap-2 rounded-full border border-gray-200 px-3 py-1.5 text-sm text-slate-600 dark:border-gray-800 dark:text-slate-300">
                        <input v-model="form.supplier_uuids" type="checkbox" :value="supplier.uuid">{{ supplier.name }}
                    </label>
                </div>
                <p v-if="editing" class="mt-1 text-xs text-slate-400">Un fournisseur qui a un prix en cours pour ce médicament reste rattaché.</p>
            </fieldset>
        </section>

        <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-900 dark:bg-gray-950">
            <h2 class="font-heading text-base font-bold text-slate-800 dark:text-white">Prix de vente</h2>
            <p v-if="editing && m.sale_price" class="mt-0.5 text-sm text-slate-500">Prix actuel : <strong>{{ formatMoney(m.sale_price) }}</strong>. Un nouveau prix s’applique aux prochaines ventes ; les ventes passées gardent le leur.</p>
            <div class="mt-4 grid gap-4 sm:grid-cols-2">
                <label class="block">
                    ...
                    <span v-if="editing && !canChangePrice" class="mt-1 block text-xs text-slate-400">Changer le prix de vente demande la permission de modifier les tarifs.</span>
                </label>
        ...
        <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            ...
            <Button type="submit" size="lg" :disabled="form.processing || (editing && !form.isDirty)">
                <Icon name="save" /><span class="ms-2">{{ form.processing ? 'Enregistrement…' : (editing ? 'Enregistrer les modifications' : 'Ajouter au catalogue') }}</span>
            </Button>
        </div>
    </form>
</template>
