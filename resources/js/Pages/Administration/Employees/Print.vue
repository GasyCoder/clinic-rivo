...
defineProps({ employee: Object, payroll: { type: Object, default: null } });
const page = usePage();
...
<template>
    ...
    <div class="employee-print-page mx-auto w-full max-w-4xl space-y-3">
        ...
        <article class="employee-print-document">
            ...
            <section class="mt-6"><h2 class="border-b border-slate-300 pb-1 text-xs font-black uppercase tracking-wider">Identité professionnelle</h2><div class="mt-3 grid grid-cols-2 gap-x-8 gap-y-3 text-sm"><p><strong>Nom et prénoms :</strong> {{ employee.name }}</p><p><strong>Fonction :</strong> {{ employee.job_title || 'N/R' }}</p><p><strong>Département :</strong> {{ employee.department || 'N/R' }}</p><p><strong>Date d’entrée :</strong> {{ formatDate(employee.hire_date) }}</p><p><strong>Ancienneté :</strong> {{ employee.seniority?.label || 'N/R' }}</p><p><strong>Diplôme :</strong> {{ employee.diploma || 'N/R' }}</p><p><strong>Niveau :</strong> {{ employee.education_level || 'N/R' }}</p><p><strong>Badge :</strong> {{ employee.badge || 'N/R' }}</p><p><strong>Blouse :</strong> {{ employee.blouse || 'N/R' }}</p></div></section>
            ...
            <!-- ADR-197 — imprimée seulement pour qui a le droit de la voir. -->
            <section v-if="payroll" class="mt-6"><h2 class="border-b border-slate-300 pb-1 text-xs font-black uppercase tracking-wider">Rémunération et banque</h2><div class="mt-3 grid grid-cols-2 gap-x-8 gap-y-3 text-sm"><p><strong>Rémunération :</strong> {{ payroll.remuneration_label || 'N/R' }}</p><p><strong>Montant mensuel :</strong> {{ payroll.remuneration_amount !== null && payroll.remuneration_amount !== undefined ? formatMoney(payroll.remuneration_amount) : 'N/R' }}</p><p><strong>Numéro de compte :</strong> <span class="font-mono">{{ payroll.bank_account_number || 'N/R' }}</span></p><p><strong>Titulaire :</strong> {{ payroll.bank_account_holder || 'N/R' }}</p></div></section>
...
<style>
.employee-print-photo { width:4cm; height:4cm; flex-shrink:0; }
