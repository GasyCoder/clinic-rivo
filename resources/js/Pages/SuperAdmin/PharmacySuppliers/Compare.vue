...
const visible = computed(() => {
    ...
    return needle
        ? props.medicines.filter((medicine) => `${medicine.name} ${medicine.code}`.toLowerCase().includes(needle))
        : props.medicines;
});

// key: `${medicine_uuid}|${supplier_uuid}` — the same medicine can be ordered
// from two suppliers at once, and each choice keeps its own quantity.
const basket = ref({});
const keyOf = (medicine, quote) => `${medicine.medicine_uuid}|${quote.supplier_uuid}`;
const inBasket = (medicine, quote) => keyOf(medicine, quote) in basket.value;

const addToBasket = (medicine, quote) => {
    const key = keyOf(medicine, quote);

    if (key in basket.value) {
...
const submit = () => {
    form.orders = bySupplier.value.map((group) => ({
        ...
        lines: group.lines.map((line) => ({ medicine_uuid: line.medicine_uuid, quantity: Number(line.quantity), unit_price: line.unit_price })),
    }));
        ...
        <template v-else>
            ...
            <section class="rounded-xl border border-border bg-card p-5 shadow-sm">
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                    ...
                    >
                        ...
                        {{ supplier.name }}
                        <span class="text-xs font-normal opacity-70">{{ supplier.offers_count }} prix</span>
                    </button>
                </div>
            </section>
            ...
            <section class="rounded-xl border border-border bg-card shadow-sm">
                ...
                <EmptyState
                    v-if="!visible.length"
                    icon="package"
                    title="Aucun prix fournisseur"
                    description="Importez un catalogue fournisseur, ou rattachez ses lignes à des médicaments, pour comparer les prix ici."
                />
                ...
                <div v-else class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            <tr v-for="medicine in visible" :key="medicine.medicine_uuid" class="border-b border-border/70 align-top last:border-0">
                                <td class="px-5 py-4">
                                ...
                                <td class="px-5 py-4">
                                    <Badge :variant="stockTone(medicine)">{{ medicine.in_stock }}<span v-if="medicine.minimum_stock"> / {{ medicine.minimum_stock }}</span></Badge>
                                </td>
                                <td class="px-5 py-4">
                                    <div class="flex flex-wrap gap-2">
                                        <button
                                            v-for="quote in medicine.quotes"
                                            :key="quote.supplier_uuid"
                                            type="button"
                                        ...
                                        >
                                            <span class="min-w-0">
                                                ...
                                                <span class="block text-xs" :class="quote.price === medicine.best_price ? 'font-bold text-emerald-600 dark:text-emerald-400' : 'text-muted-foreground'">
                                                    {{ formatMoney(quote.price) }}<span v-if="quote.price === medicine.best_price && medicine.quotes.length > 1"> · le moins cher</span>
                                                </span>
                                            </span>
                    ...
                    <div v-for="group in bySupplier" :key="group.supplier_uuid" class="px-5 py-4">
                        ...
                        <div class="mt-3 space-y-2">
                            <div v-for="line in group.lines" :key="line.key" class="flex flex-wrap items-center gap-3">
                                ...
                                <span class="text-sm tabular-nums text-muted-foreground">{{ formatMoney(line.unit_price) }}</span>
                                <input
