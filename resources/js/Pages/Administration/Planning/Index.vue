...
const filter = () => router.get(hrUrl('/administration/planning'), { from: from.value, to: to.value }, { preserveState: true, replace: true });
...
<template>
    ...
    <div class="space-y-5">
        <HrPageHeader eyebrow="Organisation des équipes" title="Planning" description="Visualisez les créneaux par journée et leurs affectations. Les éventuels chevauchements restent visibles sans blocage automatique." icon="calender-date" tone="sky">
            <template #actions><Button v-if="can('planning.print')" :as="Link" :href="hrUrl(`/administration/planning/print?${periodQuery}`)" size="rg" variant="white-outline"><Icon name="printer" /><span class="ms-2">Imprimer</span></Button><Button v-if="can('planning.export')" as="a" :href="hrUrl(`/administration/planning/export?${periodQuery}`)" size="rg" variant="white-outline"><Icon name="download" /><span class="ms-2">Exporter</span></Button><Button v-if="can('planning.create')" :as="Link" :href="hrUrl('/administration/planning/create')" size="rg"><Icon name="plus" /><span class="ms-2">Nouveau créneau</span></Button></template>
        </HrPageHeader>
