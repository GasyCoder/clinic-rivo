...
<template>
    ...
    <div class="space-y-5">
        ...
        <form class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-900 dark:bg-gray-950" @submit.prevent="apply">
            ...
            <div class="grid gap-3 p-5 sm:grid-cols-[1fr_1fr_auto]"><div><label for="report_from" class="mb-1.5 block text-xs font-bold text-slate-500">Période du</label><DatePicker id="report_from" v-model="from" /></div><div><label for="report_to" class="mb-1.5 block text-xs font-bold text-slate-500">Au</label><DatePicker id="report_to" v-model="to" /></div><Button class="self-end" size="rg"><Icon name="check" /><span class="ms-2">Appliquer</span></Button></div>
        </form>
