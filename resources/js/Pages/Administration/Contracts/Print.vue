...
const generateHref = (template) => `/administration/generated-documents/create?${new URLSearchParams({
...
<template>
    ...
    <div class="mx-auto max-w-4xl space-y-4">
        <div class="print-actions flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <Button :as="Link" href="/administration/contracts" size="rg" variant="white-outline"><Icon name="arrow-left" /><span class="ms-2">Retour aux contrats</span></Button>
            ...
            <div v-if="documents.length" class="mt-5 border-t border-gray-100 pt-4 dark:border-gray-900">
                ...
                <ul class="mt-2 divide-y divide-gray-100 dark:divide-gray-900">
                    <li v-for="document in documents" :key="document.uuid" class="flex items-center justify-between gap-3 py-2 text-sm">
                        ...
                        <Button v-if="can('generated_documents.print')" :as="Link" :href="`/administration/generated-documents/${document.uuid}/print`" size="sm" variant="white-outline"><Icon name="printer" /><span class="ms-1.5">Réimprimer</span></Button>
                    </li>
                </ul>
            </div>
        </section>
