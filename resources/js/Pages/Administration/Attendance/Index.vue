...
const filter = () => router.get(hrUrl('/administration/attendance'), { from: from.value, to: to.value, employee: employee.value || undefined }, { preserveState: true, replace: true });
...
<template>
    ...
    <div class="space-y-5">
        <HrPageHeader eyebrow="Temps constaté" title="Présences" description="Consultez les sessions d’entrée et de sortie réellement enregistrées, sans calcul automatique de retard ni d’heures supplémentaires." icon="clock" tone="violet">
            <template #actions><Button v-if="can('attendance.print')" :as="Link" :href="hrUrl(`/administration/attendance/print?${queryString}`)" size="rg" variant="white-outline"><Icon name="printer" /><span class="ms-2">Imprimer</span></Button><Button v-if="can('attendance.export')" as="a" :href="hrUrl(`/administration/attendance/export?${queryString}`)" size="rg" variant="white-outline"><Icon name="download" /><span class="ms-2">Exporter</span></Button><Button v-if="can('attendance.create')" :as="Link" :href="hrUrl('/administration/attendance/create')" size="rg"><Icon name="plus" /><span class="ms-2">Nouvelle présence</span></Button></template>
        </HrPageHeader>
