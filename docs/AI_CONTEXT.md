...
`light_*` / `dark_*` background et foreground) ; `ThemePalette` en déduit les surfaces
et n'écrit dans `<style id="rivo-theme">` que ce qui est réglé (vide = apparence
d'origine). Préréglages dans `ThemePresets` ; un avant-plan à moins de 4,5 de contraste
est refusé (`ReadableThemeColors`) ; `utilities/themePalette.js` reproduit le calcul
serveur pour l'aperçu (test). Réglages avancés du site (`ui_font_size`, `ui_density`,
`ui_radius`, `ui_motion`, `ui_contrast`, `UiOptions`) posés sur `<html>` au rendu
serveur ; chacun ajuste taille du texte, animations et contraste dans « Mon profil » ›
Apparence (`PUT /profil/apparence`, `users.ui_preferences`, sur le compte et non le
poste) ; `AppSettings::appearance($user)` → prop partagée `appearance`, réappliquée
par `applyAppearance()`. Numéros : `AppSettings::patientNumbering()` →
`PatientNumberFormat` (préfixe, année none/2/4, chiffres, séparateur, remise annuelle
ou compteur continu clé 0, chiffres du rang) ; sans réglage = ADR-030 ; les générateurs
sautent tout numéro existant et n'en réécrivent aucun (`peek()` pour l'aperçu). Matricule :
`EmployeeNumberAllocator` propose le suivant du modèle (`EMP-0001`), archives comprises ;
vide à la création ou à l'import = la proposition. Les paramètres suivent la page
« Settings » de shadcn/ui : `/super-admin/settings` redirige vers le premier module ;
`/super-admin/settings/{module}?site=`, sur toute la largeur, montre le titre et le choix
du site réglé en un clic (`SettingsSiteSwitcher`, état de chaque site), puis le
module ouvert dans une carte bordée et conteneur (`cq`, variantes `cq-4xl:` qui suivent
la largeur de la carte ; Ctrl+S enregistre) (icône et groupe du module en tête, champs empilés
`SettingsSection` + `SettingsField` ; `Input`, `Select`, `Tabs`, `RadioGroup`, `Switch`),
son pied collant portant l'état de la saisie et « Enregistrer », et à sa droite le
menu des modules en carte, rangé par groupe avec les icônes (`SettingsNav`, liste
`utilities/settingsSections.js` = `AppSettingsController::SECTIONS`, module inconnu → 404 ;
au-dessus du module, en ligne qui défile, sur un écran étroit). Le formulaire garde toutes
les valeurs du site : le même `PUT` les renvoie, aucune règle ni API ne change. Aucune
permission nouvelle.

**Mode maintenance d'un site** (ADR-193). Les sites seulement (le portail n'a que des Super Admin). `site_maintenances` : une ligne par maintenance, jamais effacée ; état lu à chaque requête — `UPCOMING` (bandeau 24 h avant), `ACTIVE` (fermé), `ENDED` (rouvert à la fin prévue), `LIFTED`. `SetSiteMaintenanceAction` / `LiftSiteMaintenanceAction` (`app_maintenance.update`, une seule ouverte à la fois, audit `app_maintenance.*`) ; `SiteMaintenanceState` (scoped) sert `site.maintenance` ; `EnforceSiteMaintenance` (groupe web) répond page `Maintenance` en 503 (+ `Retry-After`, JSON si attendu), sauf connexion/déconnexion/mot de passe, branding, robots.txt, `/up`, l'API du site, et les comptes `app_maintenance.bypass`. Portail : Paramètres › Maintenance (`MaintenanceSettings`, aperçu `MaintenanceNotice` partagé avec la page), par `PUT /api/v1/super-admin/app-settings/maintenance` et `POST …/lift`.

**Remises** (ADR-192, amende ADR-133). Une seule remise par facture, la plus avantageuse, calculée sur la part patient (après mutuelle et prise en charge Personnel). Sources : remise propre au patient (`patient_discounts`, accordée/annulée par `discounts.approve` depuis la carte « Remises » du dossier), personnel (patient relié à un employé en poste, règle `staff_discount_*`), VIP (ADR-133, remise `patient_vip_settings.discount_*` réglée avec les seuils dans Patients VIP, sans effet si la catégorie est désactivée), coupon (`discount_coupons`, code saisi à la Caisse). Pourcentage ou montant (`DiscountType`, plafonné à la part patient). `InvoiceDiscountResolver` choisit, `ApplyInvoiceDiscountAction` / `RemoveInvoiceDiscountAction` (`discounts.create`) posent et retirent, seulement sur une facture brouillon ou à encaisser sans paiement ; `invoice_discounts` garde l'instantané (une active par facture, `active_key`), `InvoiceDiscountTotals` recalcule (aussi à l'ajout d'une ligne) ; ramenée à zéro, la facture passe `COVERED` sans paiement. Caisse : `InvoiceDiscountPanel` dans la fenêtre d'encaissement. Portail : module « Remises » des Paramètres (règle personnel `app_settings.staff_discount_*` avec le reste, coupons créés/archivés par `/api/v1/super-admin/app-settings/coupons`, `discount_coupons.*` ; un coupon archivé jamais utilisé se supprime définitivement — `DeleteDiscountCouponAction`, `discount_coupons.force_delete`, corbeille désactivée avec sa raison s'il a servi) ; la remise VIP se règle dans Patients VIP (`PatientVipSettingsController::rules()`, partagées portail/site). Le crédit du personnel reste le crédit Bloc RH (ADR-052).

**Le Super Admin gère les RH d'un site depuis le portail** (ADR-187, corrige l'écart avec le CDC §18). `routes/hr.php` porte les 65 routes RH une seule fois, servies sous `/administration` (site) et `/api/v1/super-admin/hr` (API, middleware `rivo.remote-actor` + `rivo.hr-screens`). Le portail relaie `/super-admin/sites/{site}/rh/...` (`SiteHumanResourcesController`, `SiteHrGateway`) et affiche la même page Vue ; composants limités aux écrans RH, liens du site réécrits vers le portail. Côté site, `RemoteSuperAdmin` (un `User` jamais enregistré, sans id, droits = ceux transmis) fait passer contrôleurs, règles et actions RH sans réécriture ; l'audit le signe par son UUID/nom, et congés, pièces, documents générés et crédit Bloc gardent `external_*_by_uuid/name`. Dans les écrans RH, toute adresse passe par `hrUrl()` ; `HrPortalBar` donne la navigation RH sur le portail, en deux niveaux : les thèmes de l'accueil RH (`HR_SECTION_GROUPS`, `groupHrSections`, source unique dans `utilities/hrSections.js`) puis les rubriques du thème ouvert, sur une ligne à partir de 77 rem de large, sur deux sinon — jamais de défilement horizontal.

**Un seul accueil RH par site au portail** (ADR-187, amendement bis du 2026-09-26). L'accueil RH d'un site est l'écran du site relayé (`/super-admin/sites/{code}/rh`) ; `/super-admin/workspaces/hr` ne garde que « Tous les sites » et redirige `?site=A` vers l'accueil du site. `SiteHrGateway::screens()` liste les écrans RH relayables — un test lit tous les `Inertia::render` des contrôleurs de `routes/hr.php` et échoue si l'un est refusé (Stages l'était). « Effectif par département » est servi par `HrOverviewService::departments()` sur l'accueil du site (`HrDepartmentHeadcount`). Au portail, la rubrique « Emails professionnels » ouvre `/super-admin/professional-emails?site=A` (`hrSections` → `PORTAL_OWN_PAGES`), l'adresse relayée y redirige. La fiche employé mène au compte (`utilities/employeeAccount.js`) : l'écran Utilisateurs s'ouvre sur `?employe=` (portail : `?site=A&employe=`).

**Départements et Fonctions ont leur module ; le compte se relie à sa fiche depuis « Utilisateurs »** (ADR-188, amende ADR-168). `/administration/departments` et `/administration/job-titles` (`HrStructureController`, page `Administration/HrStructure/Index`, servis au portail par ADR-187) gèrent le même référentiel que les Paramètres RH (`hr_reference_values`) avec les mêmes actions et droits `hr_settings.*` ; le type se lit sur l'adresse (`HrStructureKind`), les Paramètres RH ne les listent plus. Le formulaire Employé n'a plus « Compte de connexion » (`user_uuid` refusé, `prohibited`). À la création d'un compte (assistant du portail et écran Utilisateurs du site, composant `AccountKindPicker`), on choisit **Personnel clinique** (une fiche Employé en poste, nom et email proposés) ou **Externe** : obligatoire à la création, omis en modification = lien inchangé (`AccountKindRules`). `EmployeeAccountLinker` pose ou retire `employees.user_id` dans la transaction de `CreateUserAction` / `UpdateUserAction` — une fiche, un compte, archives comprises ; audit `user.employee.link` / `.unlink`. Le type n'est jamais stocké : il se lit sur le lien (`AccountKind`). Pour « Personnel clinique », on **cherche d'abord la personne** (`AccountKindPicker`, auto-complétion sur les fiches déjà servies, `utilities/staffSearch.js`, clavier ↑ ↓ Entrée Échap) ; Nom et Email n'apparaissent qu'après, repris de la fiche et modifiables ; une fiche sans email laisse le champ vide et y place le curseur ; passer à « Externe » vide ce qui venait d'une fiche (ADR-188, amendement du 2026-09-25).

**RH : fonctions par département, photo 4 × 4, stagiaires, planning de garde** (ADR-194). `hr_job_title_departments` relie une fonction aux départements où elle existe ; le formulaire employé ne propose que les fonctions du département choisi (groupées, une fonction sans lien reste proposée partout, le couple déjà enregistré reste choisissable) et `JobTitleDepartmentGuard` refuse un couple incohérent à la saisie comme à l'import. La correspondance se règle dans le module Fonctions (`HrStructureController`, `department_uuids` via `HrStructureRequest`, `SyncJobTitleDepartmentsAction`, audit `hr_reference.departments.update`, composant `JobTitleDepartmentsPicker`) ; la proposition livrée (`DefaultJobTitleDepartments`, CDC §9) ne relie qu'une fonction encore sans département. Photo : `EmployeePhotoStore` recoupe au carré, 600 px, JPEG sans métadonnées, disque privé, servie par `GET /administration/employees/{uuid}/photo` (`view-employee-photo`) ; `EmployeePhotoField` / `EmployeePhoto`. Stagiaire = employé + contrat dont le type porte `metadata.internship` (Paramètres RH) : filière (`INTERNSHIP_FIELD`), école, niveau, encadrant ; page `/administration/internships` (`InternshipController`, `InternshipDirectory`), « Nouveau stagiaire » = `employees/create?stagiaire=1` puis contrat de stage. Planning : `planning_shifts.kind` SHIFT (service) / ON_CALL (garde), onglets Personnel / Garde, vues semaine, mois, liste (`utilities/planningCalendar.js`). Aucune permission nouvelle.

**Rémunération et ancienneté du dossier employé** (ADR-197, amende ADR-066/069 : une déclaration, aucune paie calculée). `employees.remuneration_type` (`EmployeeRemunerationType` SALARY / ALLOWANCE / UNPAID), `remuneration_amount` (mensuel, exigé sauf UNPAID, effacé pour UNPAID), `bank_account_number`, `bank_account_holder` (exigé avec un numéro). Droits `employees.payroll.view` / `.update` (ADMINISTRATION par défaut) : sans `view`, le bloc `payroll` (servi à part par `HrPresenter::payroll()`, jamais dans `employee()`) est `null` ; sans `update`, champs `prohibited` et `EmployeePayroll::prepare()` refuse dans les actions ; omis = inchangés. Ancienneté calculée depuis la date d'entrée (`App\Support\Hr\Seniority`, `utilities/seniority.js`), servie en `employee.seniority`. Le formulaire repère ses étapes par clé ; l'étape « Rémunération » n'existe qu'avec `.update`. Carte `EmployeePayrollCard` sur la fiche, section sur l'impression ; ni export ni import. La page RH du portail (`/super-admin/workspaces/hr`) s'ouvre sur l'accueil RH du premier site connecté, avec la même grille `HrAreaBoard` que le site ; `?site=all` = comparatif (ADR-187, amendement du 2026-09-26).

**RH cohérentes** (ADR-198). Un stagiaire n'est pas un employé : `InternshipDirectory::interns()` / `withoutInterns()` (contrat en cours ou à venir tous des stages ; sinon dernier contrat terminé = stage) filtrent la liste Employés, ses compteurs et `HrOverviewService` (actifs, effectif par département). `LeaveToday` = congé **accepté** couvrant la date : `on_leave` sur chaque ligne d'employé, `?status=on_leave`, chiffre `on_leave_today` (`leave.view`). L'impression d'un congé propose les canevas `EMPLOYEE_AND_LEAVE` et ouvre `generated-documents/create?employee=&leave=` (un seul canevas → choisi d'office). Présences : onglet « Aujourd'hui » par défaut (`AttendanceTodayBoard` : PRESENT, EXPECTED, LEFT, ON_LEAVE, OFF), historique dès `from/to/employee/open` ou `?vue=historique`. Jamais `window.`/`new URLSearchParams` dans un gabarit Vue (test).

**Documents en dossiers** (ADR-199). `App\Support\Documents\DocumentFamily` range un type de document dans un dossier (sans accents ni casse ; CONTRAT, CONGE, ATTESTATION, CERTIFICAT, LETTRE, DECISION, AUTRE, puis les types libres) ; `utilities/documentFamilies.js` la reproduit à l'écran. Site : `/administration/generated-documents` = dossiers, `?dossier=KEY` = canevas du dossier + documents (`&statut=archives`), `?q=` traverse tout. Un document ne se réécrit pas : « Modifier » = `create?from=<uuid>` puis `replaces_uuid` → `CreateGeneratedDocumentAction` archive l'ancien (« Remplacé par une nouvelle version. », `replaces_document_id`) ; « Supprimer » = `DELETE` avec motif (`ArchiveGeneratedDocumentAction`, `generated_documents.archive`) ; `POST …/restore` (`generated_documents.restore`), refusé si une version plus récente est en vigueur. `EmploymentContractController::print` ouvre le document déjà produit pour le contrat, sinon la génération si un seul canevas de contrat est actif ; `?choisir=1` garde l'écran de choix. Portail : `/super-admin/workspaces/document-templates?site=A&dossier=KEY` (dossiers côté écran, `FolderCard` sans `href` émet `open`), `create?type=KEY` → `preset` (type + contexte), un canevas créé retombe dans son dossier.

**Adresses email professionnelles** (ADR-190). Le RH du site demande depuis la fiche employé (carte `ProfessionalMailboxCard`, `professional_emails.request`, adresse proposée par `ProfessionalEmailAddress::suggest()` : prenom.nom sans accents, chiffre si homonyme, domaine `rivo.professional_email.domain`) → `professional_mailboxes` REQUESTED (`ProfessionalMailboxWorkflow`, jamais supprimée, `active_key` / `employee_active_key` uniques). Le Super Admin crée sur `/super-admin/professional-emails` (`ProfessionalEmailController`) : relit la demande, crée la boîte chez l'hébergeur (`CpanelMailboxClient`, UAPI `Email::add_pop` en POST, `Authorization: cpanel user:token`), retient ce qu'il a fait (`professional_mailbox_provisions`, jamais de mot de passe), puis active sur le site (`/api/v1/super-admin/professional-mailboxes/{uuid}/activate`) — l'email de la fiche devient l'adresse. Mot de passe (`MailboxPassword`) montré une fois dans la réponse JSON, jamais stocké ni journalisé. Suspension (`suspend_login`) au départ, jamais de suppression ; « À suspendre » liste les employés partis. Accès à l'hébergeur dans le `.env` du portail (`RIVO_MAIL_HOSTING_URL/_USER` et `_TOKEN`, à défaut `_PASSWORD` — l'offre o2switch n'ouvre pas les jetons, et refuse l'authentification Basic sur l'API : le client ouvre alors une session cPanel, `POST /login/?login_only=1` puis `/cpsessNNN/execute/Email/…`, gardée chiffrée dans le cache `RIVO_MAIL_HOSTING_SESSION_MINUTES` (10) et rejouée une fois si elle a expiré ; `POST …/professional-emails/prepare` l'ouvre d'avance à l'ouverture d'une fenêtre ; la double authentification du compte cPanel doit rester désactivée) et, **si on le souhaite, aussi d'un site** : un RH à qui le Super Admin a accordé `professional_emails.create/.reject/.deactivate/.activate/.update` agit alors depuis la page RH `/administration/professional-emails` ; sans ces accès sur le site, il ne fait que voir et demander. L'email d'un employé ne se saisit plus (amendement du 2026-09-25) : `email` est refusé par `EmployeeDataRequest`, jamais écrit par Create/UpdateEmployeeAction, ignoré par l'import et absent du modèle Excel ; seule l'activation de l'adresse pro le pose, et une modification de fiche ne l'efface pas. Un seul chemin : `MailboxProvisioner` avec `RemoteMailboxRegistry` (portail → API du site) ou `LocalMailboxRegistry` (site) ; un seul écran : `ProfessionalEmailsWorkspace`. Par défaut ADMINISTRATION a `.view/.request`.

**Messagerie : la boîte pro de son titulaire, lue en direct** (ADR-195, complète ADR-190). `/messagerie` dépend des permissions (amendement du 2026-09-25) : `webmail.view` fait apparaître le menu et ouvre SA boîte — l'adresse ACTIVE de la fiche employé reliée au compte (socle des rôles des sites, sauf SUPPORT/MAINTENANCE) ; `webmail.open_any` ouvre la boîte d'un autre employé du site (aucun rôle d'un site par défaut). Le mot de passe de la boîte reste exigé. **Le portail n'ouvre qu'une boîte : la sienne** (amendement du 2026-09-26 ter) — `RIVO_WEBMAIL_PORTAL_ADDRESS` / `_PASSWORD` / `_NAME` dans son .env, construite par `WebmailAccess::portalBox()` (drapeau `portal`), mot de passe rendu par `WebmailAccess::password()` (le .env pour elle, la session pour les autres) ; le Super Admin arrive directement dans la boîte de réception (`/messagerie/connexion`, « changer » compris, redirige), rien à fermer, `webmail.connect` audité une fois par session ; `canOpenAny()` est faux sur le portail. Les adresses actives des sites restent proposées comme destinataires (`siteAddresses()`, API, cache 5 min). Non réglée → `Webmail/Unavailable` `portal_unconfigured` ; mot de passe refusé → `Webmail/Offline` qui nomme la variable, sans boucle (le rendu de `bootstrap/app.php` reçoit la boîte courante). Dans « À », ce qui est tapé compte dès qu'il contient « @ », et une adresse valable hors collègues est proposée « Écrire à … » (`recipientSuggestions()`). `WebmailAccess` (middleware `webmail.access`) décide ; `WebmailBox` est la boîte choisie, gardée avec son mot de passe dans `WebmailSession` ; sans boîte, `Webmail/Unavailable` dit pourquoi (`permission`, `unlinked`, `no_address`, `inactive`). Ouvertures et envois depuis la boîte d'un autre portent `titular`/`own_mailbox` dans l'audit. Le mot de passe de l'adresse est vérifié par le serveur (`WebmailSessionController`), gardé chiffré dans la session seulement (`WebmailSession`), jamais en base ni dans un journal ; oublié à « Fermer ma boîte » et à la déconnexion. `webmail.open` (`OpenWebmailMailbox`) connecte `ImapMailServer` (webklex/php-imap sans ext-imap ; SMTP `EsmtpTransport`) et lie `WebmailMailbox` ; mot de passe changé → redemandé, serveur injoignable → `Webmail/Offline` (503). Rien n'est copié : seuls `webmail_labels` (mots-clés IMAP) et `webmail_templates` sont en base, par compte. Dossiers en français (rôle SPECIAL-USE ou nom, dossier manquant créé), Favoris = étoilés de Réception/Archives/Envoyés, actions groupées (`POST /messagerie/actions`, suppression définitive seulement depuis corbeille/indésirables, auditée), envoi (`/envoyer`, audit `webmail.send` sans le corps, copie dans Envoyés avec la Cci), brouillons, réponse/transfert, `return_to` limité à `/messagerie/…` (`WebmailReturn`). HTML reçu nettoyé (`EmailHtmlSanitizer::forDisplay`, images distantes bloquées) puis affiché dans un iframe sandbox sans script (`EmailBodyFrame`) ; pièces jointes toujours téléchargées. Écran : `Pages/Webmail/Index.vue` + `Components/Webmail/*`, règles d'affichage dans `utilities/webmail.js`. Config `RIVO_WEBMAIL_*` par site. Tests avec `Tests\Support\Webmail\FakeMailServer(Factory)` ; `ImapMailServerIntegrationTest` contre GreenMail si `RIVO_WEBMAIL_TEST_SERVER`. **Chaque aller-retour compte** (~260 ms Madagascar ↔ o2switch, amendement du 2026-09-25) : `LeanImapClient` supprime le NOOP de contrôle de la bibliothèque, `MailServerFactory::open()` rend un `LazyMailServer` qui ne se connecte qu'au premier usage, `ImapMailServer::pipeline()` envoie ensemble les commandes indépendantes (STATUS de tous les dossiers, STORE + MOVE/EXPUNGE), APPEND en LITERAL+ ; arborescence (2 min), quota (10 min) et capacités (1 jour) en cache sous une clé hôte + adresse — jamais un message ; SMTP 465 par défaut, PLAIN avant LOGIN, QUIT après la réponse. Mesuré : dossier 1,5 s (56 s avant), message 1,7 s (21 s), envoi 2,7 s (10 s). **Rien n'attend plus** (amendement du 2026-09-26) : le contrôleur annonce ce que la page lira (`WebmailMailbox::expectListing()` / `expectMessage()` → `MailServer::plan()`, seulement si la page demande `list` ou `message`), et ces lectures partent avec le STATUS des compteurs — un message ne coûte plus que la connexion + un aller-retour (lecture et `\Seen` dans le même lot ; lectures d'avance oubliées à toute modification). Écran : liens `WEBMAIL_NAVIGATION` (`preserveState`, `except: WEBMAIL_STATIC_PROPS`), pas de squelette pleine page ; états `PendingMessage` (en-tête repris de la ligne) et `PendingList` ; retour à la liste par l'historique (`rememberOpening` / `canReturnByHistory`, intercepté par `onBefore` car le `Link` d'Inertia écrase `@click`) puis relecture en arrière-plan ; dossiers et pages voisines préchargés au survol (30 s, étiquette `webmail`, oubliée après chaque action, envoi, ouverture) ; envoi en arrière-plan en JSON (200 / 422 / 409 `reconnect`), message rendu intact à la fenêtre s'il est refusé. `Webmail/Connect.vue` en pleine largeur (grille, flèches, boîte choisie à droite), sur un site seulement — le portail ne l'affiche plus. **Boîte de réception lisible d'un coup d'œil** (amendement du 2026-09-26 bis) : `listing()` renvoie `counts {all, unseen, flagged}` (UID SEARCH FLAGGED dans le même lot ; `null` pour « Favoris » réuni et une liste en erreur), affichés en pastilles rouges sur « Tous / Non lus / Favoris » et mis à jour tout de suite par `applyActionLocally` (`countsDelta`) ; non-lus en rouge dans la colonne des dossiers, brouillons neutres ; le bandeau « Boîte d'un employé » devient une pastille ambre permanente qui s'ouvre sur l'explication d'audit ; changer de boîte et la fermer sont deux icônes de la carte.

**La Pharmacie d'un site se lit et s'administre depuis le portail** (ADR-189, étend ADR-187, applique ADR-098). `routes/pharmacy.php` porte les routes une seule fois, servies sous `/pharmacy` (site) et `/api/v1/super-admin/site-pharmacy` (API, `rivo.remote-actor` + `rivo.hr-screens`) ; le portail relaie `/super-admin/sites/{site}/pharmacie/...` (`SitePharmacyController`, `SitePharmacyGateway`, `pharmacy.view`, arrivée sur « Médicaments & stock ») et affiche la même page Vue. Le relais RH et Pharmacie partagent `SiteScreenGateway` / `SiteScreenController`. Les actes physiques — délivrer, ticket, servir un consommable, entrer en stock, inventaire, ajustement, réceptionner, rupture ligne à ligne — portent `rivo.site-only` (`KeepPhysicalActsAtSite`) : refusés au Super Admin distant **même avec le droit**, montrés verrouillés à l'écran (`SiteOnlyAction`), jamais masqués. L'administratif (médicaments, prix de vente, familles, fournisseurs, catalogues, commandes, factures) passe par `CatalogActor::fromUser()`, qui reconnaît `RemoteSuperAdmin` et signe par UUID/nom (`external_*`, audit). Dans les écrans Pharmacie, toute adresse passe par `pharmacyUrl()` ; `PharmacyPortalBar` donne les rubriques du site (`pharmacySections`). Un test JS interdit une adresse `/pharmacy` nue dans ces écrans.
...
**Le Super Admin du portail détient réellement toutes les permissions** (ADR-186). Seul `RolePermissionSeeder` le faisait, et il ne se rejoue plus (ADR-064) : 18 permissions manquaient à la base du portail et 26 au rôle, si bien que « Canevas de documents » avait disparu de son menu et que le site refusait ces commandes (le portail transmet ses droits dans `X-Rivo-Actor-Permissions`). `SyncPortalSuperAdminPermissionsAction` s'exécute à chaque `php artisan migrate` du portail, même sans migration en attente (`MigrationsEnded`, `NoPendingMigrations`) : il crée les permissions du catalogue qui manquent et les accorde toutes au `SUPER_ADMIN`, sans rien retirer ni toucher aux autres rôles ou aux exceptions (un `DENY` garde la priorité) ; audit `role.permissions.portal_sync`. Sans effet sur un site clinique (ADR-027). Signalé : l'en-tête atteint 7,7 Ko, proche des 8 Ko d'un nginx par défaut.
...
seuil configuré, personne n'est VIP. Aucun tarif ni droit n'en dépend ; une remise VIP peut être réglée avec les seuils (ADR-192).
...
Tout bouton « Actualiser » ou « Réessayer » affiche `Components/Shadcn/RefreshIcon.vue` (`:spinning`) : au moins
un tour complet (700 ms, `@keyframes rivo-refresh-turn`), arrêt en fin de tour lu sur `animationiteration`, filet
si les animations réduites (ADR-191) coupent la rotation. Plus aucune icône d'actualisation ne tourne par un
`animate-spin` écrit à la main — un test le garde (ADR-195, amendement du 2026-09-26 bis).
