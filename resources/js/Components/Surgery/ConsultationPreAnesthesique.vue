                        ...
                        <div class="grid grid-cols-1 divide-y divide-amber-100 dark:divide-amber-950 lg:grid-cols-3 lg:divide-x lg:divide-y-0">
                            ...
                            <div class="space-y-4 p-4">
                                ...
                                <div class="grid grid-cols-1 gap-2 sm:grid-cols-3">
                                    <label class="text-xs text-slate-500">Grossesses (G)<Input v-model="form.consultation_data.gyneco_obstetric.gravida" v-bind="fieldAttrs('consultation_data.gyneco_obstetric.gravida')" type="number" min="0" max="30" /><FormError v-if="errorMessage('consultation_data.gyneco_obstetric.gravida')" :id="errorId('consultation_data.gyneco_obstetric.gravida')" :message="errorMessage('consultation_data.gyneco_obstetric.gravida')" /></label>
                    ...
                    <section class="overflow-hidden rounded-md border border-orange-100 dark:border-orange-950">
                        <div class="grid grid-cols-1 gap-5 p-4 xl:grid-cols-3">
                            <div class="space-y-3">
                                ...
                                <label v-for="field in [['cardiovascular', 'Cardio-vasculaire'], ['pulmonary', 'Pulmonaire'], ['neurological', 'Neurologique'], ['coloration', 'Coloration'], ['venous_access', 'Abord veineux'], ['spinal_access', 'Abord rachidien']]" :key="field[0]" class="block text-xs text-slate-500">{{ field[1] }}<Input v-model="form.consultation_data.clinical_exam[field[0]]" v-bind="fieldAttrs(`consultation_data.clinical_exam.${field[0]}`)" /><FormError v-if="errorMessage(`consultation_data.clinical_exam.${field[0]}`)" :id="errorId(`consultation_data.clinical_exam.${field[0]}`)" :message="errorMessage(`consultation_data.clinical_exam.${field[0]}`)" /></label>
                            </div>
                            <div class="space-y-3">
                                ...
                                <div class="grid grid-cols-2 gap-2"><label class="text-xs text-slate-500">FC<Input v-model="form.consultation_data.heart_rate" v-bind="fieldAttrs('consultation_data.heart_rate')" type="number" min="20" max="300" placeholder="batt/min" /><FormError v-if="errorMessage('consultation_data.heart_rate')" :id="errorId('consultation_data.heart_rate')" :message="errorMessage('consultation_data.heart_rate')" /></label><label class="text-xs text-slate-500">SpO₂<Input v-model="form.consultation_data.oxygen_saturation" v-bind="fieldAttrs('consultation_data.oxygen_saturation')" type="number" min="0" max="100" placeholder="%" /><FormError v-if="errorMessage('consultation_data.oxygen_saturation')" :id="errorId('consultation_data.oxygen_saturation')" :message="errorMessage('consultation_data.oxygen_saturation')" /></label></div>
                                <label class="block text-xs text-slate-500">FR<Input v-model="form.consultation_data.respiratory_rate" v-bind="fieldAttrs('consultation_data.respiratory_rate')" type="number" min="1" max="150" placeholder="cycles/min" /><FormError v-if="errorMessage('consultation_data.respiratory_rate')" :id="errorId('consultation_data.respiratory_rate')" :message="errorMessage('consultation_data.respiratory_rate')" /></label>
                            </div>
                            <div class="space-y-3">
                                <h4 class="text-xs font-bold uppercase text-slate-400">Voies aériennes et prothèses</h4>
                                <label v-for="field in [['mouth_opening', 'Ouverture buccale'], ['mallampati', 'Mallampati'], ['thyromental_distance', 'Distance thyromentonnière'], ['cervical_spine', 'Rachis cervical'], ['dental_prosthesis', 'Prothèse dentaire'], ['other_prosthesis', 'Autre prothèse']]" :key="field[0]" class="block text-xs text-slate-500">{{ field[1] }}<Input v-model="form.consultation_data[field[0]]" v-bind="fieldAttrs(`consultation_data.${field[0]}`)" /><FormError v-if="errorMessage(`consultation_data.${field[0]}`)" :id="errorId(`consultation_data.${field[0]}`)" :message="errorMessage(`consultation_data.${field[0]}`)" /></label>
                            </div>
                        </div>
                    </section>
