...
import { Activity, ArrowLeft, ArrowRight, Check, List, Pencil, Save } from 'lucide-vue-next';
...
const selectClass = 'block h-10 w-full appearance-none rounded-lg border border-border bg-card px-3 pe-9 text-sm text-foreground outline-none transition focus:border-primary focus:ring-2 focus:ring-ring/25 ';
...
<template>
    <form id="analysis-wizard" class="scroll-mt-4 space-y-4" @submit.prevent="submit">
        ...
        <section class="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
            <div class="h-1 bg-muted"><div class="h-full bg-primary transition-all duration-300" :style="{ width: progress }" /></div>
                    ...
                    <li v-for="(step, index) in steps" :key="step.number">
                        <button
                            ...
                            :class="[
                                ...
                                currentStep === step.number ? 'bg-primary/5 ' : 'hover:bg-accent/50',
                                step.number > maxStepReached ? 'cursor-not-allowed opacity-45' : '',
                            ]"
                            @click="goToStep(step.number)"
                        >
                            <span :class="['flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold', currentStep === step.number ? 'bg-primary text-white' : step.number < currentStep ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-muted text-muted-foreground ']">
                                <Check class="h-4 w-4" v-if="step.number < currentStep" /><span v-else>{{ step.number }}</span>
                            </span>
                            <span class="min-w-0"><span :class="['block truncate text-sm font-bold', currentStep === step.number ? 'text-primary' : 'text-foreground']">{{ step.label }}</span><span class="mt-0.5 block truncate text-[11px] text-muted-foreground">{{ step.hint }}</span></span>
                        </button>
                    </li>
                </ol>
            </nav>
        ...
        <div class="flex items-center justify-between gap-3 px-1">
            <div class="flex items-center gap-2.5"><span class="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 text-primary"><component :is="lucideIcon(currentMeta.icon)" /></span><div><p class="text-sm font-bold text-foreground">{{ currentMeta.label }}</p><p class="text-[11px] text-muted-foreground">Étape {{ currentStep }} sur {{ steps.length }} · * obligatoire</p></div></div>
            <span class="hidden text-xs font-bold text-primary sm:block">{{ progress }}</span>
        </div>

        <section v-if="currentStep === 1" class="grid overflow-hidden rounded-2xl border border-border bg-card shadow-sm lg:grid-cols-[320px_minmax(0,1fr)]">
            <aside class="border-b border-border bg-muted/70 p-5 /40 lg:border-b-0 lg:border-e">
                <span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/15 text-xl text-primary"><Activity class="h-4 w-4" /></span>
                <h2 class="mt-4 font-heading text-lg font-bold text-foreground">Où se situe cette analyse ?</h2>
                <p class="mt-2 text-sm leading-6 text-muted-foreground">La prestation Laboratoire porte la facturation ; cette fiche ne structure que le résultat. Le niveau détermine si un groupe parent est possible, obligatoire, ou exclu.</p>
            </aside>
            <div class="grid content-start gap-4 p-5 sm:grid-cols-2 sm:p-6">
                <div class="sm:col-span-2"><label for="catalog_item_uuid" class="mb-1.5 block text-sm font-medium text-foreground">Prestation Laboratoire <span class="text-red-500">*</span></label><select id="catalog_item_uuid" v-model="form.catalog_item_uuid" :class="selectClass" :aria-invalid="Boolean(form.errors.catalog_item_uuid)" @change="form.parent_uuid = ''"><option v-for="item in catalogItems" :key="item.uuid" :value="item.uuid">{{ item.code }} · {{ item.name }}</option></select><FormError v-if="form.errors.catalog_item_uuid">{{ form.errors.catalog_item_uuid }}</FormError></div>
                <div><label for="code" class="mb-1.5 block text-sm font-medium text-foreground">Code <span class="text-red-500">*</span></label><Input id="code" v-model="form.code" placeholder="NFS-HB" :aria-invalid="Boolean(form.errors.code)" /><FormError v-if="form.errors.code">{{ form.errors.code }}</FormError></div>
                <div><label for="level" class="mb-1.5 block text-sm font-medium text-foreground">Niveau <span class="text-red-500">*</span></label><select id="level" v-model="form.level" :class="selectClass" :aria-invalid="Boolean(form.errors.level)" @change="form.parent_uuid = ''"><option v-for="level in levels" :key="level" :value="level">{{ levelLabel(level) }}</option></select><p class="mt-1.5 text-xs text-muted-foreground">{{ levelHint(form.level) }}</p><FormError v-if="form.errors.level">{{ form.errors.level }}</FormError></div>
                <div v-if="mayHaveParent(form.level)" class="sm:col-span-2"><label for="parent_uuid" class="mb-1.5 block text-sm font-medium text-foreground">{{ form.level === 'PARENT' ? 'Groupe parent (facultatif)' : 'Groupe parent' }} <span v-if="form.level === 'CHILD'" class="text-red-500">*</span></label><select id="parent_uuid" v-model="form.parent_uuid" :class="selectClass" :aria-invalid="Boolean(form.errors.parent_uuid)"><option value="">{{ form.level === 'PARENT' ? 'Aucun — groupe racine' : 'Choisir' }}</option><option v-for="parent in availableParents" :key="parent.uuid" :value="parent.uuid">{{ parent.path }}</option></select><FormError v-if="form.errors.parent_uuid">{{ form.errors.parent_uuid }}</FormError></div>
            </div>
        </section>

        <section v-else-if="currentStep === 2" class="grid overflow-hidden rounded-2xl border border-border bg-card shadow-sm lg:grid-cols-[320px_minmax(0,1fr)]">
            <aside class="border-b border-border bg-sky-50/60 p-5 dark:bg-sky-950/10 lg:border-b-0 lg:border-e">
                <span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-sky-100 text-xl text-sky-700 dark:bg-sky-950 dark:text-sky-300"><Pencil class="h-4 w-4" /></span>
                <h2 class="mt-4 font-heading text-lg font-bold text-foreground">Comment se présente le résultat ?</h2>
                <p class="mt-2 text-sm leading-6 text-muted-foreground">Le type de résultat conditionne la saisie côté laboratoire (valeur numérique, texte libre, choix ou oui/non).</p>
            </aside>
            <div class="grid content-start gap-4 p-5 sm:grid-cols-2 sm:p-6">
        ...
        <section v-else-if="currentStep === 3" class="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
            <header class="flex flex-col gap-2 border-b border-border bg-muted/70 px-5 py-4 /40 sm:flex-row sm:items-center sm:justify-between"><div><h2 class="text-sm font-bold text-foreground">Références et valeurs</h2><p class="mt-1 text-xs leading-5 text-muted-foreground">Tout est facultatif ; à valider selon la méthode et les réactifs du laboratoire.</p></div><span class="w-fit rounded-full bg-muted px-3 py-1 text-[11px] font-bold text-muted-foreground">Étape facultative</span></header>
        ...
        <section v-else-if="currentStep === 4" class="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
            <header class="flex flex-col gap-2 border-b border-border bg-muted/70 px-5 py-4 /40 sm:flex-row sm:items-center sm:justify-between"><div><h2 class="text-sm font-bold text-foreground">Sous-analyses</h2><p class="mt-1 text-xs leading-5 text-muted-foreground">Uniquement pour un niveau « Groupe » — chaque sous-analyse peut à son tour être un sous-groupe.</p></div><span class="w-fit rounded-full bg-muted px-3 py-1 text-[11px] font-bold text-muted-foreground">Étape facultative</span></header>
            <div class="p-5 sm:p-6">
                <SubAnalysesEditor
                    ...
                    :max-depth="1"
                />
                <p v-else class="rounded-lg border border-dashed border-input px-4 py-8 text-center text-sm text-muted-foreground dark:border-border">Cette analyse est de niveau « {{ levelLabel(form.level) }} » : elle ne peut pas avoir de sous-analyses. Choisissez « Groupe » à l’étape Classification pour en ajouter.</p>
            </div>
        </section>

        <section v-else class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
            <div class="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                <header class="border-b border-border bg-muted/70 px-5 py-4 /40"><h2 class="text-sm font-bold text-foreground">Définition prête à enregistrer</h2><p class="mt-1 text-xs text-muted-foreground">Contrôlez les informations principales avant de valider.</p></header>
            ...
            <aside class="h-fit overflow-hidden rounded-2xl border border-emerald-200 bg-emerald-50 shadow-sm dark:border-emerald-900 dark:bg-emerald-950/20"><div class="p-5"><span class="flex h-11 w-11 items-center justify-center rounded-full bg-emerald-600 text-xl text-white"><Check class="h-4 w-4" /></span><h2 class="mt-4 font-heading text-lg font-bold text-emerald-900 dark:text-emerald-100">À retenir</h2><ul class="mt-3 space-y-3 text-sm leading-5 text-emerald-800 dark:text-emerald-200"><li class="flex gap-2"><Check class="mt-0.5 shrink-0 h-4 w-4" /><span>La prestation Laboratoire porte le tarif ; cette fiche ne structure que le résultat.</span></li><li class="flex gap-2"><Check class="mt-0.5 shrink-0 h-4 w-4" /><span>Une sous-analyse retirée de la liste est désactivée à l’enregistrement, jamais supprimée.</span></li><li class="flex gap-2"><Check class="mt-0.5 shrink-0 h-4 w-4" /><span>Un troisième niveau se crée séparément, rattaché via « Groupe parent ».</span></li></ul></div></aside>
        </section>
