...
const filter = () => router.get(hrUrl('/administration/contracts'), { q: query.value || undefined, status: status.value }, { preserveState: true, replace: true });
const archiveForm = (uuid) => archiveForms.value[uuid] ??= useForm({ reason: '' });
const archive = (contract) => archiveForm(contract.uuid).delete(hrUrl(`/administration/contracts/${contract.uuid}`));
...
<template>
    ...
    <div class="w-full space-y-5">
        <HrPageHeader eyebrow="Cycle contractuel" title="Contrats du personnel" description="Suivez les engagements, leurs modèles documentaires, leurs périodes et leurs archives, sans automatisation de paie." icon="file-docs" tone="sky">
            <template #actions><Button v-if="can('contracts.export')" as="a" :href="hrUrl('/administration/contracts/export')" size="rg" variant="white-outline"><Icon name="download" /><span class="ms-2">Exporter</span></Button><Button v-if="can('contracts.create')" :as="Link" :href="hrUrl('/administration/contracts/create')" size="rg"><Icon name="plus" /><span class="ms-2">Nouveau contrat</span></Button></template>
        </HrPageHeader>

        <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <Link v-for="item in stats" :key="item.key" :href="hrUrl(`/administration/contracts?status=${item.key}`)"><HrStatCard :label="item.label" :value="summary[item.key]" :hint="item.hint" :icon="item.icon" :tone="item.tone" :active="status === item.key" /></Link>
        </section>
