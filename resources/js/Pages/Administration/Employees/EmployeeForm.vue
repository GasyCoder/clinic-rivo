...
const props = defineProps({
    ...
    jobTitles: Array,
    addresses: Array,
    /** ADR-168 — comptes de connexion reliables à la fiche. */
...
const stepFields = {
    ...
    2: ['employee_number', 'department_uuid', 'job_title_uuid', 'hire_date', 'user_uuid'],
...
// ADR-168 — le compte relié rend le planning RH lisible (disponibilité d'un
// chirurgien au bloc). Il ne crée aucun compte et ne donne aucun droit.
const accountOptions = computed(() => [
    { value: '', label: 'Aucun compte relié' },
    ...props.accounts.map((account) => ({
        value: account.uuid,
        label: [account.name, account.profile ?? account.role].filter(Boolean).join(' — ') + (account.available ? '' : ' (désactivé)'),
    })),
]);
const selectedAccount = computed(() => props.accounts.find((account) => account.uuid === props.form.user_uuid)?.name || 'Aucun compte relié');
    ...
    <form id="employee-wizard" class="scroll-mt-4 space-y-4" @submit.prevent="submit">
        ...
        <section v-if="currentStep === 1" class="grid overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-900 dark:bg-gray-950 lg:grid-cols-[320px_minmax(0,1fr)]">
            ...
            <div class="p-5 sm:p-6">
                ...
                <div class="mt-6 grid gap-4 sm:grid-cols-2">
                    ...
                    <div><label for="birth_date" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-white">Date de naissance</label><DatePicker id="birth_date" v-model="form.birth_date" /><FormError v-if="form.errors.birth_date">{{ form.errors.birth_date }}</FormError></div>
        ...
        <section v-else-if="currentStep === 2" class="grid overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-900 dark:bg-gray-950 lg:grid-cols-[320px_minmax(0,1fr)]">
            ...
            <div class="grid content-start gap-4 p-5 sm:grid-cols-2 sm:p-6">
                ...
                <div><label for="hire_date" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-white">Date d’entrée</label><DatePicker id="hire_date" v-model="form.hire_date" /><FormError v-if="form.errors.hire_date">{{ form.errors.hire_date }}</FormError></div>
                ...
                <FormField as="div" class="sm:col-span-2" label="Compte de connexion" hint="(facultatif)" :error="form.errors.user_uuid">
                    <ShadSelect id="user_uuid" v-model="form.user_uuid" :options="accountOptions" :icon="KeyRound" placeholder="Aucun compte relié" class="w-full" aria-label="Compte de connexion" />
                    <p class="mt-1.5 text-xs leading-5 text-muted-foreground">Relie cette fiche au compte qui se connecte : son planning RH dit alors quand la personne est disponible — par exemple un chirurgien à la programmation du bloc. Aucun compte n’est créé et aucun droit n’est donné.</p>
                </FormField>
            </div>
        </section>
