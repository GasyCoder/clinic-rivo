...
class UpdateMedicineDiagnosisRequest extends FormRequest
{
    ...
    public function after(): array
    {
        return [function (Validator $validator): void {
            ...
            if ($diagnosis?->consultation?->episode_orientation_id !== $orientation->getKey()) {
                ...
                $validator->errors()->add('diagnosis_id', 'Un diagnostic annulé ou rectifié ne peut plus être modifié.');
            } elseif ($diagnosis->recorded_by !== $this->user()?->getKey()) {
                $validator->errors()->add('diagnosis_id', 'Seul l’auteur de ce diagnostic peut le modifier.');
            }
        }];
    }
    ...
    {
        return ['description.required' => 'Saisissez le diagnostic corrigé.'];
    }
}
