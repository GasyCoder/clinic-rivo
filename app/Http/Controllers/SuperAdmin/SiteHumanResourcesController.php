 ...
 */
class SiteHumanResourcesController extends Controller
{
    public function __invoke(Request $request, string $site, SiteHrGateway $gateway, string $path = ''): SymfonyResponse|InertiaResponse
    {
        ...
        if (array_key_exists('component', $json)) {
            abort_unless($gateway->isHrScreen($json['component']), 404);
    ...
    private function screen(array $page, SiteHrGateway $gateway, array $target, string $base): InertiaResponse
    {
        return Inertia::render($page['component'], [
            ...
            'hrContext' => [
                ...
                'overview_url' => route('super-admin.workspaces.hr'),
            ],
        ]);
    }

    /** @param array<string, mixed> $json */
    private function redirect(array $json, SiteHrGateway $gateway, string $base): RedirectResponse
    {
        $target = $gateway->portalUrl((string) $json['redirect'], $base);

        // Une redirection hors de l'espace RH ramène à son accueil.
    ...
    private function unavailable(Request $request, string $message): SymfonyResponse
    {
        ...
        return redirect()->route('super-admin.workspaces.hr')->with('error', $message);
    }
