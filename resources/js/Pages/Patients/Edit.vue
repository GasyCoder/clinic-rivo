                    ...
                    <div class="p-5">
                        <div class="grid gap-x-4 gap-y-5 lg:grid-cols-12">
                            <label class="lg:col-span-2">
                                ...
                                <select :class="selectLgClass" :value="form.civility ?? ''" @change="chooseCivility"><option value="">Choisir</option><option v-for="option in civilityOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select>
                                <FormError v-if="form.errors.civility" class="mt-1">{{ form.errors.civility }}</FormError>
                            </label>
                            <label class="lg:col-span-5"><span class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-white">Nom <span class="text-red-500">*</span></span><IconInput v-model="form.last_name" size="lg" icon="user" autocomplete="family-name" required /><FormError v-if="form.errors.last_name" class="mt-1">{{ form.errors.last_name }}</FormError></label>
                            <label class="lg:col-span-5"><span class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-white">Prénom(s)</span><IconInput v-model="form.first_name" size="lg" icon="user" autocomplete="given-name" /><FormError v-if="form.errors.first_name" class="mt-1">{{ form.errors.first_name }}</FormError></label>

                            <fieldset class="lg:col-span-4">
                                <legend class="mb-1.5 text-sm font-medium text-slate-700 dark:text-white">Sexe <span class="text-red-500">*</span></legend>
                                <div class="flex h-11 items-center gap-5 rounded-md border border-gray-200 bg-white px-4 dark:border-gray-800 dark:bg-gray-950"><label class="inline-flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300"><input v-model="form.sex" type="radio" value="M" class="text-primary-600 focus:ring-primary-200" />Masculin</label><label class="inline-flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300"><input v-model="form.sex" type="radio" value="F" class="text-primary-600 focus:ring-primary-200" />Féminin</label></div>
                            ...
                            <div class="lg:col-span-8">
                                ...
                                <div class="grid gap-2 sm:grid-cols-[150px_minmax(0,1fr)]"><select v-model="form.identity_document_type" :class="selectLgClass"><option :value="null">Type</option><option value="CIN">CIN</option><option value="PASSPORT">Passeport</option></select><IconInput v-model="form.identity_document_number" size="lg" icon="card-view" :disabled="!form.identity_document_type" placeholder="Numéro du document" /></div>
                ...
                <section class="overflow-hidden rounded-lg border border-gray-200 bg-white dark:border-gray-900 dark:bg-gray-950">
                    ...
                    <div class="space-y-5 p-5">
                        <div class="grid gap-5 md:grid-cols-2">
                            <label><span class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-white">Téléphone</span><IconInput v-model="form.phone" size="lg" icon="call" type="tel" autocomplete="tel" /><FormError v-if="form.errors.phone" class="mt-1">{{ form.errors.phone }}</FormError></label>
                            <label><span class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-white">Email</span><IconInput v-model="form.email" size="lg" icon="mail" type="email" autocomplete="email" /><FormError v-if="form.errors.email" class="mt-1">{{ form.errors.email }}</FormError></label>
                            <div class="md:col-span-2">
                                ...
                                <select v-if="!showNewAddress" :class="selectLgClass" :value="form.address_entry_uuid" @change="selectAddress"><option value="">Adresse non renseignée</option><option v-for="address in addressEntries" :key="address.uuid" :value="address.uuid">{{ address.label }}</option></select>
                                <IconInput v-else v-model="form.new_address_label" size="lg" icon="map-pin" placeholder="Saisissez la nouvelle adresse" />
